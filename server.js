...
// ✅ FIXED CORS (Vercel + localhost support)
const allowedOrigins = [
  'http://localhost:5173',
  'http://localhost:3000',
  'https://vet-flow18.vercel.app'
];
...
app.use(cors({
  origin: function (origin, callback) {
    ...
    if (allowedOrigins.includes(origin)) {
      ...
      return callback(null, true); // allow all fallback
    }
  },
  credentials: true
}));
...
io.on('connection', socket => {
  ...
  socket.on('ambulance:register', vehicleId => {
    socket.join(`amb:${vehicleId}`);
...
server.listen(PORT, '0.0.0.0', () =>
  console.log(`🚀 VetFlow running on port ${PORT}`)
);